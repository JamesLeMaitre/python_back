...
# from app.db.init_db import user, db
...
#         if result is None:
#             return None
#         return {"id": result[0], "firstname": result[1], "lastname": result[2], "address": result[3],
...
#         return formatter(False, "Internal Server Error: {}".format(str(e)), None)
#
#
...
#     try:
#         new_user_id = User(db).add_user(user)
#     except Exception as e:
#         response.status_code = 500
#         return formatter(False, "Internal Server Error: {}".format(str(e)), None)
#
#     new_user = {"id": new_user_id, **user.dict()}
